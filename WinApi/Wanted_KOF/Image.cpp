...
void Image::Render(HDC hdc, int destX, int destY, int frameIndex, bool isFlip)
{
    ...
    if (isFlip && isTransparent)
    {
        StretchBlt(imageInfo->hTempDC, 0, 0,
            ...
            imageInfo->hMemDC,
            (imageInfo->frameWidth * imageInfo->currFrameX) + (imageInfo->frameWidth + 1), // 여기까지?
            imageInfo->frameHeight * imageInfo->currFrameY, // 여기까지
            -imageInfo->frameWidth, imageInfo->frameHeight, // 이 크기로?
            SRCCOPY
        );

        GdiTransparentBlt(hdc,
            destX - imageInfo->frameWidth, destY, // 여기가 원래 destX 였음.
